...
const filterTypes = ['filter', 'multi-filter', 'multiFilter', 'keyword-filter', 'keywordFilter', 'between-filter', 'betweenFilter'];
          ...
          this.filteredData = this.query_result.data.rows.filter(row =>
            filters.reduce((memo, filter) => {
              ...
              return (memo && some(filter.current, (v) => {
                ...
                // 范围搜索
                if (['betweenFilter', 'between-filter'].includes(filter.filterType)) {
                ...
                // 关键字模糊搜索
                if (['keywordFilter', 'keyword-filter'].includes(filter.filterType)) {
